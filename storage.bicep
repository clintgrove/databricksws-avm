...
module storageAccount2 'br/public:avm/res/storage/storage-account:0.6.0' = {
  name: '${uniqueString(deployment().name, 'uksouth')}-storageaccount2-deploy'
  params: {
    name: '${uniqueString(resourceGroup().id)}lakestore2'
    allowBlobPublicAccess: false
    blobServices: {
      automaticSnapshotPolicyEnabled: true
      containerDeleteRetentionPolicyDays: 10
    ...
    localUsers: [
      {
        ...
        hasSshKey: true
        hasSshPassword: false
        homeDirectory: 'dbricksdefault'
        name: 'testuser'
        permissionScopes: [
          {
            permissions: 'r'
            resourceName: 'dbricksdefault'
            service: 'blob'
          }
        ]
        storageAccountName: '${uniqueString(resourceGroup().id)}lakestore2'
      }
    ]
    location: 'uksouth'
    managedIdentities: {
    ...
    privateEndpoints: [
      {
        ...
        service: 'blob'
        subnetResourceId:  resourceId('Microsoft.Network/virtualNetworks/subnets', 'dwwaf-vnet', 'public-subnet')
        tags: {
          Environment: 'Non-Prod'
          Role: 'DeploymentValidation'
        }
